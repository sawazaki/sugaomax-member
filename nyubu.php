...
// ── config.php書き込みヘルパー ────────────────────────────────
function write_config_nyubu(string $enroll_hash, string $token, int $active): bool
{
    global $config_path;
    $content = "<?php\n// このファイルはGitignore済みです。直接編集しないでください。\n"
        . "define('ADMIN_PASSWORD_HASH', "       . var_export(defined('ADMIN_PASSWORD_HASH')      ? ADMIN_PASSWORD_HASH      : '', true) . ");\n"
        . "define('APP_PASSWORD_HASH', "         . var_export(defined('APP_PASSWORD_HASH')        ? APP_PASSWORD_HASH        : '', true) . ");\n"
        . "define('VIEWER_PASSWORD_HASH', "      . var_export(defined('VIEWER_PASSWORD_HASH')     ? VIEWER_PASSWORD_HASH     : '', true) . ");\n"
...
function cur_enroll(string $const, string $default = ''): string {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'toggle_active') {
        if (!defined('ENROLLMENT_PASSWORD_HASH') || ENROLLMENT_PASSWORD_HASH === '') {
            $error = '先に入部届けパスワードを設定してください。';
        } else {
            $cur_active = defined('ENROLLMENT_ACTIVE') ? (int)ENROLLMENT_ACTIVE : 1;
            $new_active = $cur_active === 1 ? 0 : 1;
            $cur_token  = cur_enroll('ENROLLMENT_TOKEN', bin2hex(random_bytes(32)));
            if (write_config_nyubu(ENROLLMENT_PASSWORD_HASH, $cur_token, $new_active)) {
                header('Location: /nyubu.php?toggled=' . $new_active);
                exit;
            }
    ...
    // ── 入部届けパスワード変更（管理者のみ） ──────────────────
    if ($action === 'save_enrollment_password') {
        require_admin();
        ...
        if (strlen($pw) < 8) {
            ...
            $cur_active = defined('ENROLLMENT_ACTIVE') ? (int)ENROLLMENT_ACTIVE : 1;
            if (write_config_nyubu($new_hash, $new_token, $cur_active)) {
                header('Location: /nyubu.php?enrollment_saved=1');
                exit;
            }
            $error = 'ファイルの書き込みに失敗しました。';
        }
    ...
    if ($action === 'regenerate_token') {
        ...
        if (!defined('ENROLLMENT_PASSWORD_HASH') || ENROLLMENT_PASSWORD_HASH === '') {
            ...
            $new_token  = bin2hex(random_bytes(32));
            $cur_active = defined('ENROLLMENT_ACTIVE') ? (int)ENROLLMENT_ACTIVE : 1;
            if (write_config_nyubu(ENROLLMENT_PASSWORD_HASH, $new_token, $cur_active)) {
                header('Location: /nyubu.php?regenerated=1');
                exit;
            }
            $error = 'ファイルの書き込みに失敗しました。';
        }
...
if (!empty($_GET['enrollment_saved'])) $msg = '入部届けパスワードとQRコードを更新しました。';
...
<body>
    ...
    <div class="container">
        <div class="flex items-center justify-between mb-16">
            <h1 class="page-title" style="margin-bottom:0">入部</h1>
        </div>
        ...
        <div style="max-width:600px;">
            ...
            <div class="card" style="margin-bottom:24px;">
                ...
                <?php else: ?>
                    <p style="font-size:13px;color:#dc2626;margin:0;">
                        パスワードが未設定のため受付停止中です。下のフォームからパスワードを設定してください。
                    </p>
                <?php endif; ?>
            </div>
            ...
            <?php if (is_admin()): ?>
                <div class="card">
                    ...
                    <form method="post">
                        <input type="hidden" name="csrf_token" value="<?= h(csrf_token()) ?>">
                        <input type="hidden" name="action" value="save_enrollment_password">
